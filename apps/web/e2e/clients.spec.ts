...
async function command(page: Page, operation: string, payload: object) {
  const response = await page.request.post("/api/clients", { headers: { origin: "http://127.0.0.1:4173" }, data: { operation, payload } });
  return { status: response.status(), body: await response.json() };
}
test("directory, validation, normal create, name/phone search, edit, archive and restore", async ({ page }) => {
...
test("phone duplicates open existing, explicitly allow separate people, and recheck on edit", async ({ page }) => {
  test.setTimeout(90000);
  await start(page);
...
test("foreign URL and forged ownership are denied; revoked membership conceals the directory", async ({ page, browser }) => {
  ...
  await expect(second.getByRole("alert")).toContainText("Müşteri bulunamadı");
